...
export default function CreateCarPage() {
  ...
  const onSubmit = async (data: any) => {
    ...
    if (!res.ok) return alert(result.error || "Erro ao cadastrar carro");
    router.push("/");
  };
  ...
  useEffect(() => {
    ...
    fetch(`https://parallelum.com.br/fipe/api/v1/carros/marcas/${selectedBrandCode}/modelos`)
      .then((res) => res.json())
  ...
  useEffect(() => {
    ...
    fetch(`https://parallelum.com.br/fipe/api/v1/carros/marcas/${selectedBrandCode}/modelos/${selectedModelCode}/anos`)
      .then((res) => res.json())
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 text-zinc-100 bg-zinc-900 shadow-md">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <InputGroup label="Ano de Fabricação" name="yearFabrication" type="number" register={register} />
        <InputGroup label="Cor" name="color" register={register} />
        <InputGroup label="Quilometragem" name="mileage" type="number" register={register} />
        <InputGroup label="Preço" name="price" type="number" register={register} />
